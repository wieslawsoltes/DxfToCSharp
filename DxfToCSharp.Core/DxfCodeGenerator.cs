...
public class DxfCodeGenerator
{
    ...
    public string Generate(DxfDocument doc, string? sourcePath, string? className = null)
    {
        ...
        // Use provided class name or default to DxfDocumentGenerator
        var finalClassName = className ?? "DxfDocumentGenerator";
        
        // Header
        GenerateHeader(sb, sourcePath);
        
        // Analyze what tables we need
        AnalyzeUsedTables(allEntities);
        ...
        GenerateTableDefinitions(sb, doc);
        
        // Generate entities
        GenerateEntities(sb, allEntities);
        
        // Footer
        sb.AppendLine();
        sb.AppendLine("        // Save or return the document");
        sb.AppendLine($"        // doc.Save(\"{finalClassName.ToLowerInvariant()}.dxf\");");
    ...
    private void GenerateHeader(StringBuilder sb, string? sourcePath)
    {
    ...
    private void AnalyzeUsedTables(List<EntityObject> entities)
    {
        foreach (var entity in entities)
        {
            ...
            if (entity is MLine mline && mline.Style != null)
                _usedMLineStyles.Add(mline.Style.Name);
        }
    }

    private void GenerateTableDefinitions(StringBuilder sb, DxfDocument doc)
    {
        // Generate layer definitions
        if (_usedLayers.Count > 0)
        {
        ...
        if (_usedLinetypes.Any(lt => lt != "Continuous" && lt != "ByLayer" && lt != "ByBlock"))
        {
        ...
        if (_usedTextStyles.Any(ts => ts != "Standard"))
        {
                ...
                if (block != null)
                {
                    ...
                    if (block.Entities.Count > 0)
                    {
                        ...
                        foreach (var entity in block.Entities)
                        {
                            ...
                            GenerateBlockEntity(sb, entity, blockName);
                        }
                        sb.AppendLine();
                    }
        ...
        if (_usedDimensionStyles.Any(ds => ds != "Standard"))
        {
        ...
        if (_usedMLineStyles.Any(ms => ms != "Standard"))
        {
    ...
    private void GenerateEntities(StringBuilder sb, List<EntityObject> entities)
    {
        sb.AppendLine("        // Entities");
        
        foreach (var entity in entities)
        {
            GenerateEntity(sb, entity);
        }
    }

    private void GenerateEntity(StringBuilder sb, EntityObject entity)
    {
        switch (entity)
        {
            case Line line:
            ...
            case MLine mline:
                ...
                break;
            default:
                sb.AppendLine($"        // Unsupported entity type: {entity.GetType().Name}");
                break;
        }
    }
    ...
    private void GenerateBlockEntity(StringBuilder sb, EntityObject entity, string blockName)
    {
        // Reuse existing entity generation methods by temporarily modifying the output
        var tempSb = new StringBuilder();
        GenerateEntity(tempSb, entity);
    ...
    private void GenerateHatch(StringBuilder sb, Hatch hatch)
    {
            ...
            for (int i = 0; i < hatch.BoundaryPaths.Count; i++)
            {
                ...
                if (path.Edges.Count > 0)
                {
                    foreach (var edge in path.Edges)
                    {
                        ...
                        if (entityObj != null)
                        {
                            GenerateBoundaryEntity(sb, entityObj);
                        }
                    }
                }
    ...
    private void GenerateBoundaryEntity(StringBuilder sb, EntityObject entity)
    {
